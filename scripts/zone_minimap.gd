...
func _criar_minimap_hud() -> void:
    ...
    style.border_color = Color(0.75, 0.62, 0.35, 0.95)
    ...
    _minimap_draw.custom_minimum_size = Vector2(190, 115)
...
func _criar_modal_mapa_mundi() -> void:
    ...
    _info_label.text = "Dica: Atravesse os portais brilhantes de saída (Norte/Sul/Leste/Oeste) ou clique em uma zona para viajar!"
...
func _on_minimap_draw() -> void:
    ...
    # Fundo circular
    _minimap_draw.draw_circle(center, radius, Color(0.1, 0.14, 0.18, 0.9))
    _minimap_draw.draw_arc(center, radius, 0.0, TAU, 32, Color(0.75, 0.62, 0.35), 2.5)
    
    # Exits Norte, Sul, Leste, Oeste
    var exits: Dictionary = _current_zone_data.get("exits", {})
    if exits.get("north", "") != "":
        _minimap_draw.draw_circle(center + Vector2(0, -radius + 4), 5.0, Color(0.2, 0.85, 1.0))
    if exits.get("south", "") != "":
        _minimap_draw.draw_circle(center + Vector2(0, radius - 4), 5.0, Color(0.2, 0.85, 1.0))
    if exits.get("east", "") != "":
        _minimap_draw.draw_circle(center + Vector2(radius - 4, 0), 5.0, Color(0.2, 0.85, 1.0))
    if exits.get("west", "") != "":
        _minimap_draw.draw_circle(center + Vector2(-radius + 4, 0), 5.0, Color(0.2, 0.85, 1.0))
    ...
    if player:
        ...
        _minimap_draw.draw_circle(p_radar, 4.5, Color(1.0, 0.85, 0.2))
